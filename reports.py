...
class Report(ctk.CTk):
    def __init__(self):
        ...
        self.labels = []
        self.check_vars = []
        self.table_list = ctk.CTkComboBox(
            self, values=tables, width=500, command=self.get_list_columns
        )
        self.table_list.pack(pady=10)
        self.control_frame = ctk.CTkFrame(self, width=1200)
        self.control_frame.pack()
        self.button = ctk.CTkButton(
            self.control_frame, text="PULL", command=self.pull_bq
        )
        self.button.pack(side="left", padx=10)
        self.select_all = ctk.CTkCheckBox(
            self.control_frame, text="Select all", command=self.run_select_all
        )  # lambda: [label.select() if self.select_all.get() else label.deselect() for label in labels]
        ...
        for column in columns:
            if (
                ...
                if df[column].dt.tz != "US/Pacific":
                    ...
                    df[f"{column}_pacific"] = (
                        df[column].dt.tz_convert("US/Pacific").dt.tz_localize(None)
                    )
    ...
    def print_checkbox(self, *args, **kwargs):
        ...
        self.query_field.insert(
            0.0, f"SELECT {checkbox_str} FROM reports.{self.table_list.get()} LIMIT 100"
        )
    ...
    def get_list_columns(self, table_name):
        ...
        for row in schema:
            self.temp_checkbox = ctk.CTkCheckBox(
                self.column_frame, text=row.name, command=self.print_checkbox
            )
