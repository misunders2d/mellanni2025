...
if __name__ == "__main__":
    ...
    d = Dataset(
        start="2022-01-01", end="2025-12-31", market="US", local_data=True, save=False
    )
    ...
    for event, dates in event_dates.items():
        ...
        sku_pre_event_average = (
            pre_event_sales.groupby("sku")["unitsOrdered"].agg("sum").reset_index()
        )
        ...
        sku_event_sales = (
            specific_event_sales.groupby("sku")["unitsOrdered"].agg("sum").reset_index()
        )
        ...
        total_event_stats = pd.merge(
            sku_pre_event_average, sku_event_sales, how="outer", on="sku"
        )
