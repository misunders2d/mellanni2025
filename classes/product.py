...
event_dates_complete = [
    date for daterange in event_dates.values() for date in daterange
]
...
class Product:
    ...
    def __init__(
        ...
        self.dataset_delete = (
            False  # delete the lines for this specific set of asins from self.dataset
        )
        self.combined_dfs = {}
        self.stats = {}
    ...
    def __update_product__(self, df):
        ...
        if "sku" in df.columns:
            sku_df = (
                df.groupby(
                    ["collection", "sub-collection", "size", "color", "marketplace"]
                )
                .agg({"sku": lambda x: x.unique().tolist()})
                .reset_index()
            )
            for i, row in sku_df.iterrows():
                ...
                if individual_product in self.individual_products:
                    self.individual_products[individual_product]["sku"].update(
                        set(row["sku"])
                    )
                else:
                    self.individual_products[individual_product] = {
                        "sku": set(),
                        "asin": set(),
                    }
                    self.individual_products[individual_product]["sku"] = set(
                        row["sku"]
                    )
        ...
        if "asin" in df.columns:
            asin_df = (
                ...
                .agg({"asin": lambda x: x.unique().tolist()})
                .reset_index()
            )
            for i, row in asin_df.iterrows():
                ...
                if individual_product in self.individual_products:
                    self.individual_products[individual_product]["asin"].update(
                        set(row["asin"])
                    )
                else:
                    self.individual_products[individual_product] = {
                    ...
                    self.individual_products[individual_product]["asin"] = set(
                        row["asin"]
                    )
    ...
    def __attach_ids__(self, df):
        ...
        for i, row in df.iterrows():
            ...
            row["asin"] = "\n".join(
                [x for x in self.individual_products[product]["asin"]]
            )
            row["sku"] = "\n".join(
                [x for x in self.individual_products[product]["sku"]]
            )
            all_rows.append(row)
    ...
    def __attach_collection__(self, df):
        ...
        if "sku" in df.columns:
            merged_sku = pd.merge(
                df,
                product_dict[
                    ["sku", "collection", "sub-collection", "size", "color"]
                ].drop_duplicates("sku"),
                on="sku",
                how="left",
            )
            result[["collection", "sub-collection", "size", "color"]] = merged_sku[
        ...
        if "asin" in df.columns:
            merged_asin = pd.merge(
                df,
                product_dict[
                    ["asin", "collection", "sub-collection", "size", "color"]
                ].drop_duplicates("asin"),
                on="asin",
                how="left",
            )
            result[["collection", "sub-collection", "size", "color"]] = merged_asin[
                ["collection", "sub-collection", "size", "color"]
            ].values

        if all(["sku" in df.columns, "asin" in df.columns]):
            result[["collection", "sub-collection", "size", "color"]] = (
                merged_sku[["collection", "sub-collection", "size", "color"]]
                .fillna(merged_asin[["collection", "sub-collection", "size", "color"]])
                .values
            )
        return result
    ...
    def __attach_marketplace__(self, df, channel_column):
        df["marketplace"] = df[channel_column].apply(
            lambda x: [
                key
                for key, value in self.dataset.channels_mapping.items()
    ...
    def _pull_orders(self):
        self.orders_df = self.dataset.orders[
            (self.dataset.orders["sku"].isin(self.skus))
            | (self.dataset.orders["asin"].isin(self.asins))
        ]
        if self.dataset_delete:
            self.dataset.orders = self.dataset.orders[
                ~self.dataset.orders.index.isin(self.orders_df.index)
            ]
        self._update_ids(self.orders_df)

    def _pull_br(self):
        self.br_df = self.dataset.br[
            (self.dataset.br["sku"].isin(self.skus))
            | (self.dataset.br["asin"].isin(self.asins))
        ]
        if self.dataset_delete:
            self.dataset.br = self.dataset.br[
                ~self.dataset.br.index.isin(self.br_df.index)
            ]
        self._update_ids(self.br_df)

    def _pull_br_asin(self):
        self.br_asin_df = self.dataset.br_asin[
            self.dataset.br_asin["asin"].isin(self.asins)
        ]
        if self.dataset_delete:
            self.dataset.br_asin = self.dataset.br_asin[
                ~self.dataset.br_asin.index.isin(self.br_asin_df.index)
            ]
        self._update_ids(self.br_asin_df)

    def _pull_inventory(self):
        self.inventory_df = self.dataset.inventory[
            (self.dataset.inventory["sku"].isin(self.skus))
            | (self.dataset.inventory["asin"].isin(self.asins))
        ]
        if self.dataset_delete:
            self.dataset.inventory = self.dataset.inventory[
                ~self.dataset.inventory.index.isin(self.inventory_df.index)
            ]
        self._update_ids(self.inventory_df)

    def _pull_inventory_history(self):
        self.inventory_history_df = self.dataset.inventory_history[
            (self.dataset.inventory_history["sku"].isin(self.skus))
            | (self.dataset.inventory_history["asin"].isin(self.asins))
        ]
        if self.dataset_delete:
            self.dataset.inventory_history = self.dataset.inventory_history[
                ~self.dataset.inventory_history.index.isin(
                    self.inventory_history_df.index
                )
            ]
        self._update_ids(self.inventory_history_df)

    def _pull_advertised_product(self):
        self.advertised_product_df = self.dataset.advertised_product[
            (self.dataset.advertised_product["sku"].isin(self.skus))
            | (self.dataset.advertised_product["asin"].isin(self.asins))
        ]
        if self.dataset_delete:
            self.dataset.advertised_product = self.dataset.advertised_product[
                ~self.dataset.advertised_product.index.isin(
                    self.advertised_product_df.index
                )
            ]
    ...
    def _pull_promotions(self):
        self.promotions_df = self.dataset.promotions[
            (self.dataset.promotions["sku"].isin(self.skus))
        ]
        if self.dataset_delete:
            self.dataset.promotions = self.dataset.promotions[
                ~self.dataset.promotions.index.isin(self.promotions_df.index)
            ]
        self._update_ids(self.promotions_df)

    def _pull_returns(self):
        self.returns_df = self.dataset.returns[
            (self.dataset.returns["sku"].isin(self.skus))
            | (self.dataset.returns["asin"].isin(self.asins))
        ]
        if self.dataset_delete:
            self.dataset.returns = self.dataset.returns[
                ~self.dataset.returns.index.isin(self.returns_df.index)
            ]
        self._update_ids(self.returns_df)

    def _pull_fees_dimensions(self):
        self.fees_dimensions_df = self.dataset.fees[
            (self.dataset.fees["sku"].isin(self.skus))
            | (self.dataset.fees["asin"].isin(self.asins))
        ]
        if self.dataset_delete:
            self.dataset.fees = self.dataset.fees[
                ~self.dataset.fees.index.isin(self.fees_dimensions_df.index)
            ]
        self._update_ids(self.fees_dimensions_df)

    def _pull_warehouse(self):
        self.warehouse_df = self.dataset.warehouse[
            self.dataset.warehouse["sku"].isin(self.skus)
        ]
        if self.dataset_delete:
            self.dataset.warehouse = self.dataset.warehouse[
                ~self.dataset.warehouse.index.isin(self.warehouse_df.index)
            ]
        self._update_ids(self.warehouse_df)

    def _pull_changelog(self):
        self.changelog_df = self.dataset.changelog[
            (self.dataset.changelog["sku"].isin(self.skus))
        ]
        if self.dataset_delete:
            self.dataset.changelog = self.dataset.changelog[
                ~self.dataset.changelog.index.isin(self.changelog_df.index)
            ]

    def _pull_incoming(self):
        self.incoming_df = self.dataset.incoming[
            self.dataset.incoming["sku"].isin(self.skus)
        ]
        if self.dataset_delete:
            self.dataset.incoming = self.dataset.incoming[
                ~self.dataset.incoming.index.isin(self.incoming_df.index)
            ]

    def _pull_pricing(self):
        self.pricing_df = self.dataset.pricing[
            self.dataset.pricing["sku"].isin(self.skus)
        ]
        if self.dataset_delete:
            self.dataset.pricing = self.dataset.pricing[
                ~self.dataset.pricing.index.isin(self.pricing_df.index)
            ]

    def _pull_cogs(self):
        self.cogs_df = self.dataset.cogs[self.dataset.cogs["sku"].isin(self.skus)]
        if self.dataset_delete:
            self.dataset.cogs = self.dataset.cogs[
                ~self.dataset.cogs.index.isin(self.cogs_df.index)
            ]
    ...
    def _calculate_br_asin(self):
        ...
        br_asin = br_asin[
            (br_asin["date"] >= self.start) & (br_asin["date"] <= self.end)
        ]
        br_asin = (
            br_asin.groupby(["country_code", "date", "asin"]).agg("sum").reset_index()
        )
        sum_cols = br_asin.columns[3:]
        agg_dict = {col: "sum" for col in sum_cols}
        agg_dict["date"] = lambda x: [len(x), min(x), max(x)]
        br_asin_sum = (
            br_asin.groupby(["country_code", "asin"]).agg(agg_dict).reset_index()
        )
        if br_asin_sum["date"].tolist():
            br_asin_sum[["# days", "min_date", "max_date"]] = br_asin_sum[
                "date"
            ].tolist()
        else:
        ...
        self.stats["br_asin_total"] = (
            br_asin_total.groupby(["country_code"]).agg("sum").reset_index()
        )
    ...
    def _calculate_br(self):
        ...
        br = (
            br.groupby(["country_code", "date", "sku", "asin"]).agg("sum").reset_index()
        )

        br_sum = (
            br.groupby(["country_code", "sku", "asin"])
            .agg(
                {
                    "unitsOrdered": "sum",
                    "unitsOrderedB2B": "sum",
                    "orderedProductSales": "sum",
                    "orderedProductSalesB2B": "sum",
                    "date": lambda x: [len(x), min(x), max(x)],
                }
            )
            .reset_index()
        )
        ...
        self.stats["br_total"] = (
            br_total.groupby(["country_code"]).agg("sum").reset_index()
        )

    def _calculate_orders(self):
        self.orders_df["pacific_date"] = pd.to_datetime(
            self.orders_df["pacific_date"]
        ).dt.date
        orders = self.orders_df[
            (self.orders_df["pacific_date"] >= self.start)
            & (self.orders_df["pacific_date"] <= self.end)
        ]
        orders = (
            orders.groupby(["pacific_date", "sku", "asin", "sales_channel"])
            .agg({"units_sold": "sum", "sales": "sum"})
            .reset_index()
        )
        orders_sum = (
            orders.groupby(["sales_channel", "sku"])
            .agg(
                {
                    "units_sold": "sum",
                    "sales": "sum",
                    "pacific_date": lambda x: [len(x), min(x), max(x)],
                }
            )
            .reset_index()
        )
        if orders_sum["pacific_date"].tolist():
            orders_sum[["# days", "min_date", "max_date"]] = orders_sum[
                "pacific_date"
            ].tolist()
        else:
        ...
        self.stats["orders_total"] = (
            orders_sum.groupby("sales_channel")[["units_sold", "sales", "# days"]]
            .agg("sum")
            .reset_index()
        )
    ...
    def _calculate_inventory(self):
        ...
        inventory = self.inventory_df[
            (self.inventory_df["date"] >= self.start)
            & (self.inventory_df["date"] <= self.end)
        ]
        ...
        self.stats["inventory_total"] = (
            inventory_total.groupby("marketplace").agg("sum").reset_index()
        )
    ...
    def _calculate_inventory_history(self):
        ...
        if not "inventory_history_df" in self.__dict__:
            self._pull_inventory_history()
        self.inventory_history_df["date"] = pd.to_datetime(
            self.inventory_history_df["date"]
        ).dt.date
        self.inventory_history = self.inventory_history_df[
            (self.inventory_history_df["date"] >= start)
            & (self.inventory_history_df["date"] <= end)
        ]
        self.inventory_history = (
            self.inventory_history.groupby(["date", "sku", "asin", "marketplace"])
            .agg("sum")
            .reset_index()
        )
        ...
        n_days = (
            min(end, today) - start
        ).days  # will be used lated to calculate isr more precisely

        aggfunc = {"in stock": "mean"}  # replace with lambda x: sum(x)/n_days
        isr_df["in stock"] = (
            isr_df["available"] > 2
        )  # replace with 'Inventory_Supply_at_FBA' to calculate total isr including inbound
        self.stats["isr_sku"] = (
            isr_df.groupby(["sku", "marketplace"]).agg(aggfunc).reset_index()
        )
        self.stats["isr_sku_asin"] = (
            isr_df.groupby(["sku", "asin", "marketplace"]).agg(aggfunc).reset_index()
        )
        self.stats["isr_total"] = (
            isr_df.groupby(["marketplace"]).agg(aggfunc).reset_index()
        )

    def _calculate_advertised_product(self):
        self.advertised_product_df["date"] = pd.to_datetime(
            self.advertised_product_df["date"]
        ).dt.date
        advertised_product = self.advertised_product_df[
            (self.advertised_product_df["date"] >= self.start)
            & (self.advertised_product_df["date"] <= self.end)
        ]
        self.stats["advertised_product_detailed"] = advertised_product
        self.stats["advertised_product_breakdown"] = (
            advertised_product.groupby(["country_code", "sku", "asin"])[
                ["clicks", "impressions", "spend", "sameSkuUnits", "sameSkuSales"]
            ]
            .agg("sum")
            .reset_index()
            .sort_values(["country_code", "asin", "sku"], ascending=False)
        )
        self.stats["advertised_product_total"] = (
            advertised_product.groupby("country_code")[
                ["clicks", "impressions", "spend", "sameSkuUnits", "sameSkuSales"]
            ]
            .agg("sum")
            .reset_index()
    ...
    def _calculate_purchased_product(self):
        self.purchased_product_df["date"] = pd.to_datetime(
            self.purchased_product_df["date"]
        ).dt.date
        purchased_product = self.purchased_product_df[
            (self.purchased_product_df["date"] >= self.start)
            & (self.purchased_product_df["date"] <= self.end)
        ]
        self.combined_dfs["purchased_product"] = purchased_product
        self.stats["purchased_product_detailed"] = (
            purchased_product.groupby(
                ["country_code", "date", "sku", "asin", "purchasedAsin"]
            )[["otherSkuUnits", "otherSkuSales"]]
            .agg("sum")
            .reset_index()
        )
        self.stats["purchased_product_breakdown"] = (
            self.stats["purchased_product_detailed"]
            .groupby(["country_code", "purchasedAsin"])[
                ["otherSkuUnits", "otherSkuSales"]
            ]
            .agg("sum")
            .reset_index()
            .sort_values(["country_code", "otherSkuSales"], ascending=False)
        )
        self.stats["purchased_product_total"] = (
            self.stats["purchased_product_detailed"]
            .groupby("country_code")[["otherSkuUnits", "otherSkuSales"]]
            .agg("sum")
            .reset_index()
            .sort_values("otherSkuSales", ascending=False)
        )

    def _calculate_promotions(self):
        self.promotions_df["pacific_date"] = pd.to_datetime(
            self.promotions_df["pacific_date"]
        ).dt.date
        promotions = self.promotions_df[
            (self.promotions_df["pacific_date"] >= self.start)
            & (self.promotions_df["pacific_date"] <= self.end)
        ]
        self.combined_dfs["promotions"] = promotions
        self.stats["promotions_detaied"] = (
            promotions.groupby(["sales_channel", "sku", "description"])[
                ["item_promotion_discount", "units_sold", "sales"]
            ]
            .agg("sum")
            .reset_index()
        )
        self.stats["promotions_sku"] = (
            promotions.groupby(["sales_channel", "sku"])[
                ["item_promotion_discount", "units_sold", "sales"]
            ]
            .agg("sum")
            .reset_index()
        )
        self.stats["promotions_total"] = (
            promotions.groupby(["sales_channel"])[
                ["item_promotion_discount", "units_sold", "sales"]
            ]
            .agg("sum")
            .reset_index()
    ...
    def _calculate_returns(self):
        self.returns_df["return_date"] = pd.to_datetime(
            self.returns_df["return_date"]
        ).dt.date
        returns = self.returns_df[
            (self.returns_df["return_date"] >= self.start)
            & (self.returns_df["return_date"] <= self.end)
        ]
        self.combined_dfs["returns"] = returns
        self.stats["returns_detaied"] = (
            returns.groupby(["country_code", "sku", "status", "detailed_disposition"])
            .agg({"quantity": "sum"})
            .reset_index()
        )
        self.stats["returns_sku"] = (
            returns.groupby(["country_code", "sku"])
            .agg({"quantity": "sum"})
            .reset_index()
        )
        self.stats["returns_total"] = (
            returns.groupby(["country_code"]).agg({"quantity": "sum"}).reset_index()
        )
    ...
    def _calculate_warehouse(self):
        ...
        warehouse = self.warehouse_df[
            (self.warehouse_df["date"] >= self.start)
            & (self.warehouse_df["date"] <= self.end)
        ]
        self.stats["warehouse_total"] = (
            warehouse[["total_wh", "QtyPhysical", "total_receiving"]].sum().to_frame().T
        )

    def _calculate_changelog(self):
        self.changelog_df["date"] = pd.to_datetime(self.changelog_df["date"]).dt.date
        changes = self.changelog_df[
            (self.changelog_df["date"] >= self.start)
            & (self.changelog_df["date"] <= self.end)
        ]
        changes["change_type"] = changes["change_type"].replace(
            "Other, please specify in notes", nan
        )

        changes["description"] = changes.apply(
            lambda row: (
                f"{row['change_type']} : {row['notes']}"
                if pd.notnull(row["change_type"]) and pd.notnull(row["notes"])
                else (
                    row["change_type"]
                    if pd.notnull(row["change_type"])
                    else row["notes"]
                )
            ),
            axis=1,
        ...
        self.stats["changes_daily"] = (
            changes[["country_code", "date", "description"]].drop_duplicates().copy()
        )
        self.stats["changes_total"] = (
            changes[["country_code", "description"]].value_counts().reset_index()
        )

    def _calculate_incoming(self):
        self.stats["incoming"] = self.incoming_df.pivot_table(
            values="QtyOrdered",
            index="sku",
            columns="year-week",
            aggfunc="sum",
        ...
        if mode == "stats":
            if self.stats:
                with pd.ExcelWriter(file_path_stats, engine="xlsxwriter") as writer:
                    ...
                    for key, df in {
                        "fees": self.fees_dimensions_df,
                        "prices": self.pricing_df,
                        "cogs": self.cogs_df,
                    }.items():
                        df.to_excel(writer, sheet_name=key, index=False)
                        mm.format_header(df, writer, key)
    ...
    def summarize(self):
        ...
        def __summarize_sales__():
            sales = (
                self.orders_df.groupby(
                    ["pacific_date", "sku", "asin", "sales_channel"]
            ...
            sales = (
                ...
                )[["units_sold", "sales", "referral_fee", "promo_discount"]]
                .agg("sum")
                .reset_index()
            )
            return sales
        ...
        def __summarize_storage_fees__():
            storage = (
                self.inventory_history_df.groupby(
        ...
        def __summarize_cogs__():
            ...
            cogs = cogs[
                cogs["date"].dt.date.between(
                    self.start - pd.Timedelta(days=30), self.end
                )
            ]

            cogs["year-month"] = (
                cogs["date"].dt.year.astype("str")
                + "-"
                + cogs["date"].dt.month.astype("str")
            )
            ...
            cogs = (
                ...
                )[["product_cost", "product_cost_local"]]
                .agg("max")
                .reset_index()
            )
            return cogs
        ...
        def __summarize_ad_spend__():
            ad_spend = (
                self.advertised_product_df.groupby(
                    ["date", "sku", "asin", "country_code"]
                )[["spend", "sameSkuUnits", "sameSkuSales", "clicks", "impressions"]]
                .agg("sum")
                .reset_index()
            )
            ad_spend = ad_spend.rename(
                columns={
        ...
        def __summarize_purchased_product__():
            purchased = (
                self.purchased_product_df.groupby(
            ...
            purchased = purchased[
                purchased["date"].dt.date.between(self.start, self.end)
            ]
        ...
        def __summarize_br__():
            br = (
                self.br_asin_df.groupby(["date", "asin", "country_code"])[
                    [
                        "browserSessions",
                        "browserSessionsB2B",
                        "mobileAppSessions",
        ...
        def __summarize_changelog__():
            ...
            changes = (
                ...
                .agg(lambda x: " | ".join(x.unique().tolist()))
                .reset_index()
            )
            return changes
        ...
        def __summarize_non_product_ads__():
            ...
            dsp = (
                self.dataset.dsp[["date", "total_cost", "totalUnitsSold", "totalSales"]]
                .groupby("date")
                .agg("sum")
                .reset_index()
            )
        ...
        def __summarize_returns__():
            orders = self.orders_df[
                ["pacific_date", "amazon_order_id", "sales_channel"]
            ].copy()
            ...
            returns = returns.rename(
                columns={"order_id": "amazon_order_id", "quantity": "returned_units"}
            )
        ...
        def __merge_summary__(
            ...
            sales["year-month"] = (
                sales["date"].dt.year.astype("str")
                + "-"
                + sales["date"].dt.month.astype("str")
            )
            ...
            sales = pd.merge(
                sales, sba, how="outer", on=["date", "marketplace", "collection"]
            )
            ...
            sales["ad_sales"] = sales[
                ["salesOwnPPC", "salesOtherPPC", "sba_Sales", "dsp_Sales"]
            ].sum(axis=1)
    ...
    def restock(self, stock_days=49, max_days=90, include_empty: int | str = 0):
        ...
        def get_product_sales():
            ...
            sales_two_weeks = (
                sales_two_weeks.groupby(
                    ["marketplace", "collection", "sub-collection", "size", "color"]
                )
                .agg({"units_sold": "sum"})
                .reset_index()
            )
            sales_two_weeks = sales_two_weeks.rename(
                columns={"units_sold": short_term_sales_col}
            )
        ...
        def get_inventory():
            ...
            inv_columns = (
                ...
                + [
                    ...
                    "total_days_of_supply_with_open_shipments",
                    "marketplace",
                ]
            )
            inv = self.stats["inventory_detailed"][inv_columns].copy()
            inv_price = self.stats["inventory_detailed"][
                ["sku", "asin", "your_price", "marketplace"]
            ].copy()
        ...
        def get_warehouse():
            incoming = self.incoming_df.pivot_table(
                values="QtyOrdered", index="sku", columns="year-week", aggfunc="sum"
            ).reset_index()
            # incoming['marketplace'] = 'US'
            wh = self.warehouse_df[
                ["sku", "total_wh", "QtyPhysical", "total_receiving"]
            ].copy()
            ...
            total_wh = pd.merge(
                wh, incoming, how="outer", on="sku", validate="one_to_one"
            )

            dictionary = (
                self.dataset.dictionary[
                    [
                        "sku",
                        "collection",
                        "sub-collection",
            ...
            us_wh = pd.merge(
                total_wh, dictionary, how="left", on="sku", validate="one_to_one"
            )
            ca_wh = us_wh.copy()
        ...
        def calculate_shipment(result, warehouse):
            ...
            incoming_columns = [
                x for x in wh_columns if re.match("2[0-9]{3}-[0-9]{1,2}", x)
            ]
            to_ship = (
                result[
                    [
                        ...
                        "size",
                        "color",
                        long_term_average_sales,
                        short_term_average_sales,
                        "average corrected",
                        "dos available",
                        "dos inbound",
                        "available",
            ...
            to_ship["to ship, units"] = (
                future_sales - to_ship["Inventory_Supply_at_FBA"]
            )
            to_ship.loc[low_stock, "to ship, units"] = (
                max_sales - to_ship["Inventory_Supply_at_FBA"]
            )
            ...
            total.loc[
                total["restockable"] == "Do not ship to Amazon", "to ship, units"
            ] = 0
            total.loc[total["total_wh"] == 0, "to ship, units"] = 0
            # total.loc[duplicates, 'to ship, units'] = 0
            if not include_empty:
                total = total[
                    (total["to ship, units"] > 0) & (total["to ship, units"].notnull())
                ]
            ...
            duplicate_products = total[
                ["marketplace", "collection", "size", "color"]
            ].duplicated(keep=False)
            if len(duplicate_products) > 0:
                total.loc[duplicate_products, "potential duplicate"] = (
                    "caution, duplicate"
                )
            else:
            ...
            cols_reordered = (
                ...
                + incoming_columns
                + ["actuality", "life stage", "restockable", "potential duplicate"]
            )
        ...
        result[long_term_average_sales] = (
            result[long_term_sales_col] / num_days / result["isr"]
        ).fillna(0)
        result.loc[result["isr"] < 0.3, long_term_average_sales] = (
            result[long_term_sales_col] / num_days
        ).fillna(0)
        result[short_term_average_sales] = (
            result[short_term_sales_col] / two_weeks_days
        ).fillna(0)
        result.loc[result["isr"] < 0.3, short_term_average_sales] = (
            result[short_term_sales_col] / two_weeks_days
        ).fillna(0)

        avg1 = (
            result[long_term_average_sales] * 0.3
            + result[short_term_average_sales] * 0.7
        )
        avg2 = (
            result[long_term_average_sales] * 0.7
            + result[short_term_average_sales] * 0.3
        )

        result["average corrected"] = pd.DataFrame(
            data={"avg1": avg1, "avg2": avg2}
        ).min(axis=1)
        result = pd.merge(
            result,
            inventory,
        ...
        result["dos available"] = (
            result["available"].fillna(0) / result["average corrected"]
        )
        result["dos inbound"] = (
            result["Inventory_Supply_at_FBA"].fillna(0) / result["average corrected"]
        )
        ...
        container_weeks = [
            x for x in self.to_ship.columns if re.match(r"[0-9]{4}-[0-9]{1,2}", str(x))
        ]
        ...
        self.wh_situation = (
            wh_situation.groupby(["collection", "size", "color", "asin"])[inv_columns]
            .agg("sum")
            .reset_index()
        )
