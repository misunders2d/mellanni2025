...
class Dataset:
    ...
    def __init__(
        ...
        market: (
            Literal["US", "CA", "GB", "UK", "MX", "FR", "DE", "IT", "ES", "*"]
            | List[str]
        ) = MARKET,
        local_data: bool = LOCAL,
        save: int | str = SAVE,
    ):
        ...
        if isinstance(market, str):
            if market == "*":
                self.channel = '","'.join(
                    [value for value in Dataset.channels_mapping.values()]
                )
        ...
        self.orders = None
        self.functions = [
            self.pull_br_asin_data,
            self.pull_br_data,
            self.pull_order_data,
            self.pull_inventory_data,
            self.pull_inventory_history,
            self.pull_advertised_product_data,
            self.pull_purchased_product_data,
            self.pull_attribution_data,
            self.pull_dsp_data,
            self.pull_sba_data,
            self.pull_sbv_data,
            self.pull_sd_data,
            self.pull_promotions,
            self.pull_returns,
    ...
    def pull_order_data(self):
        ...
        if self.local_data:
            result = pd.DataFrame()
            chunks = self.__read_local__(
                os.path.join(user_folder, "orders.csv"), chunksize=100000
            )
            for chunk in chunks:
    ...
    def pull_inventory_history(self):
        ...
        if self.local_data:
            result = self.__read_local__(
                os.path.join(user_folder, "inventory_history.csv")
            )
        else:
            query = f"""SELECT DATE(snapshot_date) AS date, sku, asin, available,
                        estimated_storage_cost_next_month,
            ...
            if self.save:
                result.to_csv(
                    os.path.join(user_folder, "inventory_history.csv"), index=False
                )
    ...
    def pull_advertised_product_data(self):
        if self.local_data:
            result = self.__read_local__(
                os.path.join(user_folder, "advertised_product.csv")
            )
        else:
            ...
            if self.save:
                result.to_csv(
                    os.path.join(user_folder, "advertised_product.csv"), index=False
                )
    ...
    def pull_purchased_product_data(self):
        if self.local_data:
            result = self.__read_local__(
                os.path.join(user_folder, "purchased_product.csv")
            )
        else:
            ...
            if self.save:
                result.to_csv(
                    os.path.join(user_folder, "purchased_product.csv"), index=False
                )
    ...
    def pull_fba_shipments_data(self):
        if self.local_data:
            ...
            if self.save:
                result.to_csv(
                    os.path.join(user_folder, "fba_shipments.csv"), index=False
                )
    ...
    def pull_promotions(self):
        ...
        if self.local_data:
            ...
            if not isinstance(self.fba_shipments, pd.DataFrame):
                PopupError(
                    "FBA Shipments data not found. Please pull FBA Shipments data first."
                )
                raise BaseException("FBA Shipments data not found.")
            shipment_item_ids = self.fba_shipments[
                ["shipment_item_id", "sku", "sales_channel"]
            ].drop_duplicates()
            amazon_purchase_dates = self.fba_shipments[
                ["amazon_order_id", "pacific_date"]
            ].drop_duplicates()
            order_sales_data = (
                self.fba_shipments[["shipment_item_id", "units_sold", "sales"]]
            ...
            result = pd.merge(
                ids_result, shipment_item_ids, how="left", on="shipment_item_id"
            )
            result = pd.merge(
                result, amazon_purchase_dates, how="left", on="amazon_order_id"
            )
            result = pd.merge(
                result, order_sales_data, how="left", on="shipment_item_id"
            )
            result.loc[
                result["shipment_item_id"].duplicated(), ["units_sold", "sales"]
            ] = 0
    ...
    def pull_returns(self):
        ...
        if self.local_data:
            ...
            amazon_order_ids = pd.DataFrame(
                self.orders["amazon_order_id"].unique().tolist(),
    ...
    def pull_warehouse(self):
        ...
        if self.local_data:
            ...
            sellable = result.query(
                'Sellable == True & BinType != "Picking" & ~BinName.str.startswith("DS")'
            )
            if len(sellable) > 0:
            ...
            receiving["total_receiving"] = receiving[
                ["QtyAvailable", "QtyPhysical"]
            ].sum(axis=1)
            warehouse = pd.merge(
                sellable,
                receiving[["date", "sku", "total_receiving"]],
                how="outer",
                on=["date", "sku"],
            )
            if self.save:
                warehouse.to_csv(
                    os.path.join(user_folder, "warehouse.csv"), index=False
                )
    ...
    def pull_changelog(
        self,
    ):  # TODO add conditional to download sku_changelogs for different markets
        changelog_markets = [x for x in self.market_list if x not in ("GB", "MX")]
        tables = {
            x: f"sku_changelog_{x.lower()}" if x != "US" else "sku_changelog"
            for x in changelog_markets
        }
        tables.pop("EU", None)
        if self.local_data:
    ...
    def pull_pricing(self):
        if self.local_data:
            ...
            result = gd.download_gspread(
                spreadsheet_id="1iB1CmY_XdOVA4FvLMPeiEGEcxiVEH3Bgp4FJs1iNmQs",
                sheet_id=0,
            )
            result = result[
                [
                    ...
                    "Sale price",
                    "Discount",
                    "Date of last event (price change)",
                    "Status",
                ]
            ]
            if self.save:
