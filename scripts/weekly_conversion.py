...
def clean_changes(changes: pd.DataFrame, dictionary: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(df) == 0:
        return df
    df.loc[df["change_type"] == "Other, please specify in notes", "change_type"] = df[
        "notes"
    ]
...
def break_by_week(result: pd.DataFrame) -> pd.DataFrame:
    ...
    result["reporting_week"] = (
        result["year"].astype(str) + "-" + result["week"].astype(str)
    )
    ...
    for week in weeks:
        temp = result[result["reporting_week"] == week][
            [
                "collection",
                "sub_collection",
                "sessions",
        ...
        result_refined = pd.merge(
            result_refined, temp, how="outer", on=["collection", "sub_collection"]
        )
...
def add_totals(result_refined: pd.DataFrame) -> pd.DataFrame:
    ...
    sum_cols = [
        x
        for x in result_refined.columns
        if any(["sessions" in x, "units" in x, "conversion" in x])
    ]
...
def plot_data(df: pd.DataFrame, num_weeks: int = 2) -> BytesIO:
    ...
    for week in weeks:
        ...
        conversion = combined[combined["reporting_week"] == week]["conversion"].values[
            0
        ]
        ax1.text(
            week,
            combined["conversion"].min(),
            f"{conversion:.2%}",
            ha="center",
            color="red",
        )
    ...
    with pd.ExcelWriter(file_name, engine="xlsxwriter") as writer:
        ...
        for col in perc_cols:
            for cell, value in enumerate(df[col]):
                if not any([value is pd.NA, value == inf, pd.isna(value)]):
                    worksheet.write(
                        cell + 1, df.columns.tolist().index(col), value, perc_format
                    )
        ...
        for col in num_cols:
            for cell, value in enumerate(df[col]):
                if not any([value is pd.NA, value == inf, pd.isna(value)]):
                    worksheet.write(
                        cell + 1, df.columns.tolist().index(col), value, num_format
                    )

        # apply green and red formats
        session_cols = sorted(
            [x for x in df.columns if "sessions week" in x], reverse=True
        )
        unit_cols = sorted([x for x in df.columns if "units week" in x], reverse=True)
        conversion_cols = sorted(
            [x for x in df.columns if "conversion week" in x], reverse=True
        )
        ...
        for cell, value in enumerate(df[session_cols[0]]):
            if not any([value is pd.NA, value == inf, pd.isna(value)]):
                if session_mask.iloc[cell]:
                    worksheet.write(
                        cell + 1,
                        df.columns.tolist().index(session_cols[0]),
                        value,
                        green_format,
                    )
                else:
        ...
        for cell, value in enumerate(df[unit_cols[0]]):
            if not any([value is pd.NA, value == inf, pd.isna(value)]):
                if unit_mask.iloc[cell]:
                    worksheet.write(
                        cell + 1,
                        df.columns.tolist().index(unit_cols[0]),
                        value,
                        green_format,
                    )
                else:
        ...
        for cell, value in enumerate(df[conversion_cols[0]]):
            if not any([value is pd.NA, value == inf, pd.isna(value)]):
                if conversion_mask.iloc[cell]:
                    worksheet.write(
                        cell + 1,
                        df.columns.tolist().index(conversion_cols[0]),
                        value,
                        green_format_perc,
                    )
                else:
...
def main():
    ...
    calendar_button = ctk.CTkButton(
        app, text="Select start date", command=calendar_popup
    )
