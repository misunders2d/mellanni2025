...
def get_files(folder, extension='xls') -> None: #create a full list of all 'xlsx/xlsm' files in internal folders
...
def count_words(title): # clean, singularize and count words in a standalone title
    ...
    clean_words = [p.singular_noun(word.lower()) or word.lower() for word in all_words if word]
    counts = {}
    for word in clean_words:
        if word not in counts:
            ...
            counts[word] += 1
    counts = {word:count for word,count in counts.items() if count > 2 and word not in num_list}
    return counts

def process_file(file_path:str): # main processing logic for a single file based on its file path
    global result
    ...
    if os.path.basename(file_path).endswith('xlsm'):
        ...
        title_col_str = 'item_name'
    for template in template_sheets: # looping over different template sheet names to find the right one
        for row in range(0,5): # looping over range of 0-5 to find the line with column names
            try:
                df = pd.read_excel(file_path, sheet_name=template, skiprows=row)
                if any([title_col_str in str(x) for x in df.columns]):
                    print(f'Found correct template: {template} and header:{row}, breaking')
                    break
            except ValueError:
                pass
            except Exception as e:
                print(e)
    sku_col = [x for x in df.columns if any(['contribution_sku' in str(x),'item_sku' in str(x)])][0]
...
class TitleDuplicateChecker(ctk.CTk):
    ...
    def __init__(self):
        ...
        file_path = ctk.filedialog.askopenfilename(title='Select a file with titles', initialdir=user_folder)
        file_obj = pd.ExcelFile(file_path)
        if 'Template' in file_obj.sheet_names:
            for i in range(0,8):
                temp_file = pd.read_excel(file_obj, sheet_name='Template', nrows=20, skiprows=i)
                if any(['item_name' in x for x in temp_file.columns.tolist()]):
                    self.file = pd.read_excel(file_obj, sheet_name='Template', skiprows=i)
                    break
        else:
            self.file = pd.read_excel(file_path)
        columns = self.file.columns.tolist()

        self.label = ctk.CTkLabel(self, text='Select a column with titles that need to be checked')
