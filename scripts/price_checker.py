...
class App(ctk.CTk):
    ...
    def download_price_list(self):
        price_list = dm.download_gspread(spreadsheet_id=PRICELIST_ID)[
            ["Sub-collection", "Size", "Price", "MSRP"]
        ]
        price_list = price_list.rename(
            columns={"Sub-collection": "collection", "Size": "size"}
        )
    ...
    def merge_files(self, dictionary, price_list, fba_inventory, sale_file, event_file):
        ...
        price_check = pd.merge(
            dictionary, price_list, how="left", on=["collection", "size"]
        )
    ...
    def main(self):
        ...
        price_check = self.merge_files(
            dictionary, price_list, fba_inventory, sale_file, event_file
        )
