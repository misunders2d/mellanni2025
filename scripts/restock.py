...
method_list = {
    name: func for name, func in Dataset.__dict__.items() if name.startswith("pull")
}
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.start_date = ctk.CTkEntry(
            self.controls_frame, placeholder_text="Enter start date"
        )
        self.start_date.bind(
            "<Button-1>", lambda event: self.on_date_click(event, target="start_date")
        )
        ...
        self.end_date = ctk.CTkEntry(
            self.controls_frame, placeholder_text="Enter end date"
        )
        self.end_date.bind(
            "<Button-1>", lambda event: self.on_date_click(event, target="end_date")
        )
        ...
        self.include_empty = ctk.CTkCheckBox(
            self.controls_frame, text="Include all SKUs in restock"
        )
        ...
        self.dataset_button = ctk.CTkButton(
            ...
            text="Load\ndataset",
            height=40,
            command=lambda: self.query_dataset(method="query"),
        )
        ...
        self.partial_update_label = ctk.CTkLabel(
            self.mid_frame, text="Update specific part"
        )
        self.partial_update_label.grid(
            row=0, column=0, pady=(0, 10), padx=10, sticky="n"
        )
        ...
        self.collections_select = ctk.CTkCheckBox(
            self.mid_frame,
            text="Select all products",
            command=lambda: self.__select_all__("products"),
            state="disabled",
        )
        ...
        # size
        self.size_select = ctk.CTkCheckBox(
            self.mid_frame,
            text="select all sizes",
            command=lambda: self.__select_all__("sizes"),
            state="disabled",
        )
        ...
        # color
        self.color_select = ctk.CTkCheckBox(
            self.mid_frame,
            text="select all colors",
            command=lambda: self.__select_all__("colors"),
            state="disabled",
        )
        ...
        self.skus_input.grid(
            row=2, column=3, columnspan=2, rowspan=3, sticky="nsew", padx=5
        )
        ...
        self.product_date_from = ctk.CTkEntry(
            self.mid_frame, placeholder_text="Product date from", width=80
        )
        self.product_date_from.bind(
            "<Button-1>",
            lambda event: self.on_date_click(event, target="product_start_date"),
        )
        self.product_date_from.insert(0, start_date)
        self.product_date_from.grid(row=0, column=3)

        self.product_date_to = ctk.CTkEntry(
            self.mid_frame, placeholder_text="Product date to", width=80
        )
        self.product_date_to.bind(
            "<Button-1>",
            lambda event: self.on_date_click(event, target="product_end_date"),
        )
    ...
    def export_product(self):
        ...
        selected_skus = [
            x
            for x in re.split(r"[,\n\r\t]+", self.skus_input.get(0.0, ctk.END).strip())
            if x
        ]
        selected_collections = [
            self.collections.get(x) for x in self.collections.curselection()
        ]
        ...
        if any([selected_skus, selected_asins]):
            ...
            if selected_skus:
                asins = (
                    self.dataset.dictionary[
                        (self.dataset.dictionary["sku"].isin(selected_skus))
                        | (self.dataset.dictionary["asin"].isin(selected_skus))
                    ]["asin"]
                    .unique()
                    .tolist()
                )
            ...
            product = Product(
                asin=asins, dataset=self.dataset, start=date_from, end=date_to
            )
            ...
            self.status_label.configure(
                text=f"Done, exported data for {len(asins)} items"
            )
    ...
    def run_dataset_query(self):
        ...
        call_method(
            self.dataset, method
        )  # call a selected method from the dropdown on dataset
        self.update_status()
    ...
    def update_status(self):
        ...
        if self.run_params.get("method") == "query":
            collections = sorted(
                self.dataset.dictionary["collection"].unique().tolist()
            )
    ...
    def __on_size_select__(self, *args):
        ...
        if selected_sizes:
            ...
            if potential_colors:
                [
                    self.colors.insert(tk.END, color)
                    for color in sorted(potential_colors)
                ]
    ...
    def __place_labels__(self, market_list):
        ...
        self.select_all = ctk.CTkCheckBox(
            self.controls_frame,
            text="All",
            command=lambda: self.__select_all__("markets"),
        )
    ...
    def __select_all__(self, target):
        if target == "markets":
            [
                x.select() if self.select_all.get() else x.deselect()
...
def get_event_sales():
    ...
    def process_sku(sku):
        print(
            f"\r{' '*150}\r{len(all_files)} out of {len(skus)} processed, working on {sku}",
            end="",
            flush=True,
        )
        ...
        for event, dates in event_dates.items():
            pre_event_sales = sku_non_event_sales[
                sku_non_event_sales["date"].between(
                    min(dates) - pd.Timedelta(days=61), min(dates), inclusive="left"
                )
            ]
            the_event_sales = sku_event_sales[
                sku_event_sales["date"].between(
                    min(dates), max(dates), inclusive="both"
                )
            ]
            sku_file[f"pre-{event} average sales"] = pre_event_sales[
                "unitsOrdered"
            ].mean()
            sku_file[f"{event} units sold"] = the_event_sales["unitsOrdered"].sum()
            sku_file[f"{event} duration, days"] = len(dates)
            sku_file[f"{event} average sales"] = (
                sku_file[f"{event} units sold"] / sku_file[f"{event} duration, days"]
            )
            sku_file[f"{event} X increase"] = (
                sku_file[f"{event} average sales"]
                / sku_file[f"pre-{event} average sales"]
            )
    ...
    dataset = Dataset(
        start="2022-01-01", end="2025-12-31", market="US", local_data=False, save=False
    )
    ...
    non_event_sales = sales[
        ~sales["date"].isin([d for value in event_dates.values() for d in value])
    ]
    event_sales = sales[
        sales["date"].isin([d for value in event_dates.values() for d in value])
    ]
