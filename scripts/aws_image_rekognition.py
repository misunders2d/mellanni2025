...
if any([AWS_ACCESS_KEY is None, AWS_SECRET is None, REGION_NAME is None]):
    raise ValueError(
        "Please set AWS_ACCESS_KEY, AWS_SECRET, and REGION_NAME in your .env file!"
    )
...
def get_file_paths(img_path):
    ...
    file_paths = [
        ...
        for file in files
        if any([".png" in file.lower(), ".jpg" in file.lower()])
    ]
    return file_paths
...
def detect_labels(photo, client):
    ...
    # draw bounding boxes
    labels_with_bounding_boxes = [
        x for x in items if x["BoundingBoxes"]["BoundingBox"] != []
    ]
    all_boxes = []
    all_labels = []
...
def detect_and_draw_labels(photo):  # updated process
    ...
    response = rekognition.detect_labels(
        Image={"Bytes": image_bytes}, MaxLabels=10, MinConfidence=0.5
    )
...
def main():
    ...
    if len(file_paths) > 0:
        ...
        print(
            "No images found in the selected folder. Please select a folder with images."
        )
