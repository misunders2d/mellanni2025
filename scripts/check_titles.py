...
class TitleChecker(ctk.CTk):
    def __init__(self):
        ...
        self.query = f"""SELECT sku, product_name
                        ...
                        """
        self.market_radio_us = ctk.CTkRadioButton(
            self, text="US", value="US", variable=self.market
        )
        self.market_radio_us.pack(pady=10)

        self.market_radio_ca = ctk.CTkRadioButton(
            self, text="CA", value="CA", variable=self.market
        )
        ...
        self.folder_button = ctk.CTkButton(
            self, text="1. Select folder with flat files", command=self.select_folder
        )
        ...
        try:
            ...
            if not full_list is None:
                ...
                conditions_reasons = [
                    ...
                    (
                        result["Flat File title"].str.contains("  ", na=False),
                        "double space in flat file",
                    ),
                    (
                        result["Listing title"].str.contains("  ", na=False),
                        "double space on Amazon",
                    ),
                    (
                        result["Flat File title"].str.contains(
                            r"(?<!\s)\(", regex=True, na=False
                        ),
                        "no space before '(' in flat file",
                    ),
                    (
                        result["Listing title"].str.contains(
                            r"(?<!\s)\(", regex=True, na=False
                        ),
                        "no space before '(' on Amazon",
                    ),
                ]
    ...
    def read_flat_files(self):
        ...
        files = [
            ...
            if ".xlsm" in os.path.basename(x) and "~" not in x
        ]
        if len(files) == 0:
        ...
        for file in files:
            try:
                ...
                cols_to_use = [
                    x
                    for x in column_file.columns
                    if any(["contribution_sku" in x, "item_name" in x])
                ]
        ...
        full_list = full_list.rename(
            columns={sku_col: "sku", name_col: "Flat File title"}
        )
        duplicates = full_list["sku"].duplicated().sum()
        if duplicates > 0:
            self.warning_label.configure(
                text=f"There are {duplicates} duplicates in the folder, please check."
            )
        return full_list
