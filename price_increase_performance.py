...
def create_reporting_week(source_df):
    ...
    df['reporting week'] = df['year'].astype(str)\
                + '-'\
                + df['week'].astype(str)\
                + '\n'\
...
d = Dataset(start="2025-01-05", end="2025-12-31", market=countries, local_data = True, save=False)
d.pull_changelog()
d.pull_br_asin_data()
d.pull_dictionary()
...
sales = sales[(~sales['date'].isin(event_dates_list))&(sales['date']>=pd.to_datetime(d.start).date())]
...
sales_asins = sales_asins.groupby(
    ['reporting week','asin','collection', 'size', 'color']
    )[stats_columns].agg('sum').reset_index()
sales_asins['conversion'] = sales_asins['unitsOrdered'] / sales_asins['sessions']
sales_asins = sales_asins.sort_values(['reporting week','unitsOrdered'], ascending = (True, False))

full_sales = pd.merge(sales_asins, changes_asins, how = 'outer', on =['reporting week','asin','collection', 'size', 'color'], validate='1:1')
...
sorted_columns = sorted(reshaped_df.columns.tolist(), key=lambda x: (int(x.split('-')[0]),int(x.split('-')[1].split('\n')[0])))
reshaped_df = reshaped_df[sorted_columns].reset_index().copy()

with pd.ExcelWriter('/home/misunderstood/temp/changes.xlsx', engine='xlsxwriter') as writer:
    ...
    format_rows(reshaped_df, 'metrics', worksheet, ['browserSessions', 'browserSessionsB2B',
           'mobileAppSessions', 'mobileAppSessionsB2B', 'sessions', 'unitsOrdered',
           'unitsOrderedB2B'], num_format)
    format_rows(reshaped_df, 'metrics', worksheet, ['orderedProductSales','orderedProductSalesB2B'], curr_format)
    format_rows(reshaped_df, 'metrics', worksheet, ['change'], color_format)

    
    for row in reshaped_df.index:
        worksheet.conditional_format(row+1, 5, row+1, maxcol, conditional_format)
